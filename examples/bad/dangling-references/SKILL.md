---
name: dangling-references
description: Use when demonstrating references that will not resolve at runtime, such as a missing relative file and paths anchored to an absolute or home location.
---

# Dangling references

See [the template](templates/missing-template.md) for the expected format.

You will also need the notes at `/home/user/notes/setup.md` and the shared
helper at `~/skills/shared/util.md`.
